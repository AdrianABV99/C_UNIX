...
void dataControlChildCode(char* pathOutput) {
    ...
    while(fgets(buffer, BUFF, inputStream) != NULL) {
        counter = 0;
        buffer[strlen(buffer) - 1] = '\0'; //remove newline
        if(strchr(buffer, ' ') != NULL) {
            // result from md5sum
            // split it with strtok
            char* p = strtok(buffer, " ");
            while(p != NULL) {
                // lineToken is of same size as p; (BUFF)
                strcpy(lineToken[counter++], p);
                if(counter == 2) {
                    // we have path and hash
                    ++nrFiles;
                    // write to output
                    writeToOutput(outputFile, lineToken[1], lineToken[0]);
                    break;
                }
                p = strtok(NULL, " ");
            }
...
int searchForPath(int outputFile, FILE* inputStream2) {
    int nrFiles = 0;
    int found = 0;
    ...
    if(!found) {
        // if not found, read until line is found
        // if we find a path that is not needed yet
        // add it to unprocessed
        while(fgets(bufferHash, BUFF, inputStream2) != NULL) {
            ++nrFiles;
            if(strchr(bufferHash, '=') == NULL) {
            ...
            } else if(strstr(bufferHash, lineToken[1]) != NULL) {
                // stop reading. we found it
                break;
            } else {
                // store it in unprocessed
                strcpy(unprocessed[unprocessedLength++], bufferHash);
            }
        }           
        // we can compare the hash from file with md5 hash  
        compareHashes(bufferHash, lineToken[0], lineToken[1], outputFile);
    }
    return nrFiles;
}
...
void dataControlChildCodeCheck(char* pathOutput) {
    ...
    while(fgets(buffer, BUFF, inputStream1) != NULL) {
        ...
        while(p != NULL) {
            ...
            counter++;
            if(counter == 2) {
                // we splitted everything from md5sum
                //consider we did not found it yet
                // we get back the nrOfFiles
                nrFiles += searchForPath(outputFile, inputStream2);
                // stop strtok
                break;
            }
            p = strtok(NULL, " ");
    ...
    close(filedesc3[0]); // close read end of third pipe
    fclose(inputStream2); // close stream
    close(filedesc2[0]); // close read end of second pipe
    // write the nrOfFiles to parent pipe
    if(write(filedesc1[1], &nrFiles, sizeof(nrFiles)) < 0) {
        perror("Error while writing to parent pipie from data-control\n");
        exit(-2);
    }

    close(filedesc1[1]); // close write end of first pipe
    // close streams
    fclose(inputStream1);
    close(outputFile);
}
